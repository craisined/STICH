...
class ResNet(nn.Module):
    def __init__(self, channels=64):
        ...
        self.network = nn.Sequential(
            nn.LeakyReLU(0.2),
            weight_norm(nn.Conv1d(channels, channels, kernel_size=25, padding="same")),
            nn.LeakyReLU(0.2),
            weight_norm(nn.Conv1d(channels, channels, kernel_size=25, padding="same")),
        )
...
class Generator(nn.Module):
    def __init__(self):
        super().__init__()
        self.network = nn.Sequential(
            weight_norm(nn.Conv1d(16, 64, kernel_size=25, padding="same")),
            ResNet(),
            nn.LeakyReLU(),
            ResNet(),
            nn.LeakyReLU(),
            ...
            nn.LeakyReLU(),
            ResNet(),
            weight_norm(nn.Conv1d(64, 16, kernel_size=25, padding="same")),
        )
